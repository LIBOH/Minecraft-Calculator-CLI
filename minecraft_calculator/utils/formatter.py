...
class OutputFormatter:
    ...
    @staticmethod
    def format_result(result: CalculationResult, recipe_manager: RecipeManager, indent: int = 0) -> str:
        lines = []
        prefix = "  " * indent
        
        # Get color for current level
        level_color = OutputFormatter.LEVEL_COLORS[min(indent, len(OutputFormatter.LEVEL_COLORS) - 1)]
        
        if indent == 0:
            lines.append(f"{level_color}制作「{result.count}」个「{result.name}」需要:{Style.RESET_ALL}")
        ...
        for ing_id, _ in result.ingredients.items():
            ...
            for child in result.children:
                if child.item_id == ing_id:
                    ...
                    break
            stack_info = OutputFormatter.format_stack(result.remaining.get(ing_id, 0), child_produced, stack_size)
            lines.append(f"{prefix}{level_color}  {ing_name}: {stack_info}{Style.RESET_ALL}")
        
        if result.children:
            lines.append(f"{level_color}{'-' * 40}{Style.RESET_ALL}")
            for child in result.children:
                if child.ingredients:
                    child_output = OutputFormatter.format_result(child, recipe_manager, indent + 1)
