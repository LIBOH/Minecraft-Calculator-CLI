...
def calc(name_or_id, count, inventory, mods):
    try:
...
def inventory_remove(name_or_id, count, file):
    try:
        ...
        resolved_name = inv._resolve_item_name(name_or_id)
        if success:
            click.echo(f"{Fore.GREEN}已移除 {count} 个 {resolved_name}{Style.RESET_ALL}")
        else:
            click.echo(f"{Fore.YELLOW}无法移除: 物品不存在或数量不足{Style.RESET_ALL}", err=True)
    except InvalidInputError as e:
        click.echo(f"{Fore.RED}错误: {e}{Style.RESET_ALL}", err=True)
    except Exception as e:
        click.echo(f"{Fore.RED}错误: {e}{Style.RESET_ALL}", err=True)
    ...
    try:
        ...
        if items:
            ...
            for item in sorted(items):
                ...
                click.echo(f"  {Fore.GREEN}{name}{Style.RESET_ALL} ({Fore.WHITE}{item}{Style.RESET_ALL})")
        else:
...
def recipe_show(name_or_id):
    try:
        ...
        if not recipes:
            click.echo(f"{Fore.YELLOW}物品 '{name_or_id}' 未找到或没有配方{Style.RESET_ALL}", err=True)
            return
...
def mod_load(mod_id):
    try:
        ...
        if success:
            ...
            click.echo(f"{Fore.YELLOW}模组 '{mod_id}' 未找到{Style.RESET_ALL}", err=True)
    except RecipeLoadError as e:
        click.echo(f"{Fore.RED}错误: {e}{Style.RESET_ALL}", err=True)
